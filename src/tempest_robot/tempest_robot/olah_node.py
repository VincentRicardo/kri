...
class MyNode(Node):
    ...
    def gyro_(self, message = String):
        self.get_logger().info("Receiving Gyro = " + message.data)
        self.gyro = message.data
        self.gyro_flag = True
    ...
    def send_calculate_message(self):
        if self.cam_flag == True and self.gyro_flag == True:
            ...
            self.get_logger().info("Mengirim Gyro: " + self.gyro + " Jarak: " + str(self.cam[0]) + " cm, Sudut: " + str(self.cam[1])) #servo_calculation(self.gyro, self.cam))
            self.cam_flag = False
            self.gyro_flag = False
